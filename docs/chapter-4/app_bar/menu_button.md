...
A menu button is a button that opens a menu when clicked. Once the menu is opened, moving the mouse over another menu button or using the keyboard to move to another menu button will close the current menu and open the new one automatically.

<img src="img/menu_button.png" width=400/>

To create a menu button, use the `appbar::MenuButton::new(...)` method or the `appbar::MenuButton::with_handle(...)` method.
...
A menu button does not have an associated method in the `AppBarEvents` trait. However, once opened, any events generated by the menu can be received via the `MenuEvents` trait.
