...
fn add_checkboxes(tab: &mut Tab, index: u32) {
    macro_rules! add {
        ($x: expr, $y: expr, $type: expr, $name: expr) => {
            let mut p = Panel::with_type(
                $name,
                LayoutBuilder::new().x($x).y($y).width(20).height(9).build(),
                panel::Type::Window,
            );
            p.add(CheckBox::with_type(
                "Checked",
                LayoutBuilder::new().x(1).y(1).width(18).build(),
                true,
                $type,
            ));
            p.add(CheckBox::with_type(
                "Un-Checked",
                LayoutBuilder::new().x(1).y(2).width(18).build(),
                false,
                $type,
            ));
            ...
            p.add(CheckBox::with_type(
                "Multi-Line\nText",
                LayoutBuilder::new().x(1).y(4).width(18).height(2).build(),
                false,
                $type,
            ));
            tab.add(index, p);
    ...
    add_description(
        tab,
        index,
        "Checkboxes are controls that have two states (checked or unchecked) - a reflection of a boolean value.",
    );
...
fn add_selectors(tab: &mut Tab, index: u32) {
    add_description(
        tab,
        index,
        "Selctors are component that allows you to chose from multiple variants (such as colors, characters, list of options or numbers).",
    );
...
fn add_text(tab: &mut Tab, index: u32) {
    add_description(
        tab,
        index,
        "These controls allow you to insert text in different formats (regular, passwords, numbers, etc)",
    );
...
fn add_containers(tab: &mut Tab, index: u32) {
    add_description(
        tab,
        index,
        "Containers are a set of special controls that contains other controls (allowing one to select how the other controls are aligned)",
    );
