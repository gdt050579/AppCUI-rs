...
impl Win {
    pub(crate) fn new() -> Self {
        ...
        w.add(label!("'Three manus: File, Edit and Help, with File enabled, and Edit menu disabled (both on the left) and Help menu on the right',d:f"));
        ...
        let mut mb = appbar::MenuButton::new("&Edit", m, 1, appbar::Side::Left);
        mb.set_enabled(false);
        w.h_edit = w.appbar_mut().add(mb);
        let m = menu!(
            "class: Win, items=[
                { &About, cmd: A },
                { Welcome, cmd: A },
        ...
        w.h_help = w.appbar_mut().add(appbar::MenuButton::new("&Help", m, 1, appbar::Side::Right));  
