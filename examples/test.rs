...
#[Window(events = MenuEvents, commands=[Help, Save,Load])]
struct MyWin {
    my_menu: Handle<Menu>,
}
impl MyWin {
    fn new() -> Self {
        ...
        m.add(menu::Command::new("&Save", key!("Ctrl+S"), mywin::Commands::Save));
        m.add(menu::Separator::new());
        m.add(menu::CheckBox::new("Help", Key::None, mywin::Commands::Help, true));
        w.my_menu = w.register_menu(m);
        w
    }
}
impl MenuEvents for MyWin {
    ...
    fn on_update_menubar(&self, menubar: &mut MenuBar) {
        menubar.add(self.my_menu);
    }

    fn on_command(&mut self, menu: Handle<Menu>, item: Handle<menu::Command>, command: mywin::Commands) {
        self.set_title("Save");
        if let Some(cmd) = self.get_menuitem_mut(menu, item) {
            cmd.set_caption("Save As ...");
        }
    }
