...
impl TetrisGame {
    ...
    fn draw_square(&self, surface: &mut Surface, x: i32, y: i32, color: Color) {
        ...
        surface.draw_rect(rect, LineType::Single, charattr!("white,black"));
        
        let fill_char = Character::with_attributes(' ', CharAttribute::with_color(Color::Black, color));
        surface.write_char(screen_x + 1, screen_y + 1, fill_char);
        surface.write_char(screen_x + 2, screen_y + 1, fill_char);
        surface.write_char(screen_x + 3, screen_y + 1, fill_char);
    }

    fn paint_board(&self, surface: &mut Surface) {
        let game_width = BOARD_WIDTH * 5;
        let game_height = BOARD_HEIGHT * 3;
        let r = Rect::with_size(0, 0, game_width as u16, game_height as u16);
    ...
    fn on_paint(&self, surface: &mut Surface, theme: &Theme) {
        ...
        match self.state {
            ...
            GameState::Playing | GameState::Paused => {
                ...
                surface.write_string(game_width + 2, 2, format!("Lines: {}", self.lines_cleared).as_str(), theme.symbol.checked, false);
                
                surface.write_string(game_width + 2, 4, "Next:", charattr!("white"), false);
                for py in 0..4 {
                    for px in 0..4 {
                        if self.next_piece.shape[py][px] {
                            let preview_x = game_width + 2 + px as i32 * 2;
                            let preview_y = 6 + py as i32;
                            let preview_char = Character::with_attributes('█', CharAttribute::with_color(self.next_piece.color, Color::Black));
                            surface.write_char(preview_x, preview_y, preview_char);
                        }
                    }
                }
