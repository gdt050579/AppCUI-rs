...
def check_angle(key: str, val: Union[float, Iterable[float]]) -> bool:
    ...
    Note:
        ...
        yaw: rotation around the z axis. Defined accoding to right-hand rule.
            Range: [-180, 180]
        pitch: rotation around the yÊ¹ axis. Defined accoding to right-hand rule.
            Range: [-90, 90]
        roll: rotation around the xÊº axis. Defined accoding to right-hand rule.
            Range: [-180, 180]
...
def wrap_to_180(angle: float) -> float:
    """Wrap an angle to range Â±180 degrees.

    Args:
        angle: angle in degrees

    Returns:
        Angle in degrees wrapped to Â±180 degrees.
    """
...
class ArrayGeometry(object):
    ...
    def get_rotation(yaw: float = 0, pitch: float = 0, roll: float = 0) -> Rotation:
        ...
        Args:
            ...
            pitch: rotation around the yÊ¹ axis
            roll: rotation around the xÊº axis
    ...
    def plot(self, elev=30, azim=-55, mic_size=25):
        ...
        for dim, label in enumerate(['xâ€²', 'yâ€²', 'zâ€²']):
...
def normalize_max(x: np.ndarray, max_db: float = 0, eps: float = 1e-16) -> np.ndarray:
    """Normalize max input value to max_db full scale (Â±1).
