...
pub(crate) fn create(input: TokenStream) -> TokenStream {
    let mut cb = ControlBuilder::new("treeview", input, POSILITIONAL_PARAMETERS, NAMED_PARAMETERS, true);
    cb.init_control_with_template("TreeView", "new", "type");
    cb.add_layout();
    cb.add_flags_parameter("flags", "treeview::Flags", &FLAGS);
    ...
    if cb.has_parameter("columns") {
        ...
        if let Some(list) = cb.get_list("columns") {
            ...
            panic!("Parameter `columns` in treeview must contains a list a columns: columns=[{{...}},{{...}},{{...}}] !");
        }
        cb.add_line(&s);
    }
