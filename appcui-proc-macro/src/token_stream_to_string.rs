    ...
    fn validate_one_string_parameter(self, name: &str)->String {
        ...
        let mut string_param = match tokens.next() {
            ...
            Some(TokenTree::Ident(ident)) => {
                panic!("The parameter provided to the '{}!' macro must be a string literal and not an identifier ('{}').", name, ident)
            }
            Some(TokenTree::Punct(punct)) => {
                panic!("The parameter provided to the '{}!' macro must be a string literal and not an punctuation character ('{}').", name, punct)
            }
            Some(TokenTree::Group(g)) => {
                if g.delimiter() == Delimiter::None {
                    ...
                    if value.starts_with("\"") && value.ends_with("\"") {
                        ...
                        panic!("The parameter provided to the '{}!' macro must be a string literal and not an group ('{}' with a None delimiter).", name, g)
                    }
                } else {
        ...
        if (!string_param.starts_with('\"')) || (!string_param.ends_with('\"')) {
            panic!("The parameter provided to the '{}!' macro must be a string literal and not another literal: '{}'", name, string_param);
        }
