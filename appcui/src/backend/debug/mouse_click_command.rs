...
use crate::{
    graphics::Point,
    input::MouseButton,
    system::{MouseButtonDownEvent, MouseButtonUpEvent, SystemEvent, MouseMoveEvent},
};
    ...
    pub(super) fn new(parser: &CommandParser) -> Result<Self, ParserError> {
        ...
        if b.is_none() {
            return Err(ParserError::new(
                "Third parameter for Mouse.Click command should an mouse button (left, right or cente)",
            ));
        }
        Ok(Self {
            x: x.unwrap(),
