...
fn check_popup_menu() {
    ...
    let script = "
        ...
        ";
    let mut a = App::new().size(Size::new(80, 24)).debug_script(script).app_bar().build().unwrap();
...
fn check_menu_checkbox_methods() {
    ...
    ";
    App::new().size(Size::new(60, 15)).debug_script(script).desktop(MyDesktop::new()).app_bar().build().unwrap().run();
}
...
fn check_more_than_128_selectable_items_keyboard_navigation() {
    ...
    ";
    let mut a = App::new().size(Size::new(60, 12)).debug_script(script).app_bar().build().unwrap();
    a.add_window(MyWin::new());
    a.run();
}
