...
use listview::Group;
use crate::ui::listview::ViewMode;
...
fn check_sort_no_groups_3_columns_with_commands() {
    ...
    ";
    let mut a = App::new().size(Size::new(60, 11)).debug_script(script).command_bar().run().unwrap();
...
fn check_fold_groups_3_columns_with_keys_and_checkboxes() {
    ...
    ";
    let mut a = App::new().size(Size::new(60, 8)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
    let mut lv =
...
fn check_size_formater_renderer_simple() {
    ...
    ";
    let mut a = App::new().size(Size::new(80, 8)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
...
fn check_on_change_current_item_event() {
    ...
    impl MyWin {
        fn new() -> Self {
            let mut w = Self {
                base: window!("Test,d:f,flags: Sizeable"),
            };
            let mut lv = listview!(
                "Person,d:f,view:Columns(3),flags:ScrollBars+SearchBar+ShowGroups,columns=[{&Name,5,Left},{&Size,5,Right},{&City,5,Center}]"
            );
            Person::populate(&mut lv);
            w.add(lv);
            w
...
fn check_status_formater_renderer() {
    ...
    ";
    let mut a = App::new().size(Size::new(100, 12)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
...
fn check_temperature_formater_renderer() {
    ...
    ";
    let mut a = App::new().size(Size::new(80, 12)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
    let mut lv =
...
fn check_on_selection_changed_event() {
    ...
    impl MyWin {
        fn new() -> Self {
            ...
            let mut lv = listview!(
                "Person,d:f,view:Columns(3),flags:ScrollBars+SearchBar+ShowGroups+CheckBoxes,columns=[{&Name,5,Left},{&Size,5,Right},{&City,5,Center}]"
            );
            Person::populate(&mut lv);
            w.add(lv);
            w
...
fn check_one_frozen_columns() {
    ...
    ";
    let mut a = App::new().size(Size::new(30, 12)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
...
fn check_two_frozen_columns() {
    let script = "
        ...
        CheckHash(0x73704811ECA7AEAD)
    ";
    let mut a = App::new().size(Size::new(35, 12)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
    let mut lv = listview!("Person,d:f,flags:ScrollBars+CheckBoxes+SearchBar,columns=[{&Name,15,Left},{&Size,6,Right},{&City,20,Center}]");
...
fn check_autoresize_currency() {
    ...
    ";
    let mut a = App::new().size(Size::new(130, 10)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
...
fn check_autoresize_area() {
    ...
    ";
    let mut a = App::new().size(Size::new(120, 10)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
    let mut l = listview!("MyItem,d:f,view:Details,flags: ScrollBars");
    l.add(MyItem {
        v1: 1,
...
fn check_status_column_methods() {
    ...
    ";
    let mut a = App::new().size(Size::new(100, 15)).debug_script(script).run().unwrap();
    let mut w = window!("Test,d:f,flags: Sizeable");
    let mut l = listview!("MyItem,d:f,view:Details,flags: ScrollBars");
    l.add(MyItem {
...
#[test]
fn check_merge_border_mode_with_columns_view() {
    let script = "
        Paint.Enable(false)
        Paint('1. Initial state')
