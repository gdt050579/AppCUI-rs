...
impl KeyPressedCommand {
    pub(super) fn new(parser: &CommandParser) -> Result<Self, ParserError> {
        ...
        if (pcount != 1) && (pcount != 2) {
            return Err(ParserError::new("KeyPress command requires one or two parameters"));
        }
        let k = parser.get_key(0);
        if k.is_none() {
            return Err(ParserError::new("First parameter for KeyPress must a know key or key combination"));
        }
        if pcount == 2 {
            ...
            if t.unwrap() < 1 {
                return Err(ParserError::new("Number of times a key should be send must be a positive (>=1) number"));
            }
            Ok(Self {
                key: k.unwrap(),
    ...
    pub(super) fn generate_event(&self, sys_events: &mut VecDeque<SystemEvent>) {
        ...
        if self.key.modifier == KeyModifier::None {
            if ((self.key.code as u8) >= (KeyCode::A as u8)) && ((self.key.code as u8) <= (KeyCode::Z as u8)) {
                ch = ((('a' as u32) + (self.key.code as u32) - (KeyCode::A as u32)) as u8) as char;
            }
            if ((self.key.code as u8) >= (KeyCode::N0 as u8)) && ((self.key.code as u8) <= (KeyCode::N9 as u8)) {
