    ...
    pub(super) fn new(parser: &CommandParser) -> Result<Self, ParserError> {
        ...
        if x1.is_none() {
            return Err(ParserError::new(
                "First parameter for Mouse.Drag command should an integer (x value) - starting point",
            ));
        }
        if y1.is_none() {
            return Err(ParserError::new(
                "Second parameter for Mouse.Drag command should an integer (y value) - starting point",
            ));
        }
        if x2.is_none() {
            return Err(ParserError::new(
                "Third parameter for Mouse.Drag command should an integer (x value) - ending point",
            ));
        }
        if y2.is_none() {
            return Err(ParserError::new(
                "Fourth parameter for Mouse.Drag command should an integer (y value) - ending point",
            ));
        }
        Ok(Self {
            x1: x1.unwrap(),
