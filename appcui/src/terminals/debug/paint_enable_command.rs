    ...
    pub(super) fn new(parser: &CommandParser) -> Result<Self, ParserError> {
        if parser.get_params_count() != 1 {
            return Err(ParserError::new(
                "Paint.Enable command has parameter (a boolean value that enables or disables painted for the entire script) !",
            ));
        }
        ...
        Err(ParserError::new(
            "Expecting a boolean value as the first parameter for Paint.Enable command  !",
        ))
    }
