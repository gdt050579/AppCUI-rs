...
impl FormatRatings {
    pub(crate) fn two_chars(empty_char: char, fill_char: char, value: u32, max_value: u32, chars: u8, buf: &mut [u8]) -> Option<&str> {
        if max_value == 0 {
            return None;
        }
        ...
        let min_size = sz_empty.max(sz_fill) * (chars as usize);
        if buf.len() < min_size {
            return None;
        }
        let pos = (value as u64) * (chars as u64) / (max_value as u64);
        let mut index = 0;
        let mut buf_pos = 0;
